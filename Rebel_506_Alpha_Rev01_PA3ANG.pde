...
const int RitReadPin        = A7;  // pin that the sensor is attached to used for a rit routine later.  WAS A0
int RitReadValue            = 0;
int RitFreqOffset           = 0;
...
const int CWSpeedReadPin    = A0;  // To adjust CW speed for user written keyer.  WAS A7
...
//
// Created 27 July 2013, PA3ANG for Ten-Tec Rebel model 506
// 
...
// Global variables
int         audioState;        // variable for reading the pushbutton status
int         loopState;         // variable for reading the pushbutton status
...
// Digital Pins
//
int         tonePin      =   6;     // Tone output pin
int         LPin         =   5;     // Left paddle input     32
int         RPin         =   3;     // Right paddle input    33
int         cwoutPin     =   33;    // TX output pin
int         audioSwitch  =   2;     // Y/N audio switch
int         loopSwitch   =   7;     // Beacon or Keyer switch
int         memSwitch    =   4;     // Memory_1 switch
int         ledPin       =   33;
char        asciibuf[5];
...
// CW tone output
#define     CW_PITCH        (800)          // CW pitch 
#define     MEMORY          ("PA3ANG")     // Call sign operator 
...
//  State Machine Defines for Keyer
...
const char txt0[22]         = "TEN-TEC Rebel   v0.1";
...
void setup() 
{
    ...
    IOShieldOled.putString("TT REBEL    v0.1");
...
void loop()     // 
{
  ...
  if (morseInput.available())
  {
    ...
    //update the CW speed â€“ new!!
    CWSpeedReadValue =  analogRead(CWSpeedReadPin); //measure the voltage 
    CW_SPEED=(CWSpeedReadValue/20);            // Calculate the new CW_Speed (wpm) and thus DOTLEN   

    if (digitalRead(memSwitch) == HIGH) { 
    lcd.setCursor(0, 2);
    lcd.print("KEYING MEMORY "); // Memory  text
    lcd.print(MEMORY); 
      sendmsg(MEMORY); 
    }
    ...
    switch (keyerState) {
      ...
      case KEYED:
        ...
        if (millis() > ktimer) {            // are we at end of key down ?
            digitalWrite(ledPin, LOW);      // turn the LED off
            noTone( tonePin );
            ktimer = millis() + DOTLEN;    // inter-element time
    ...
    // Send Beacon message and stay in the loop
    // Display mode and beaconmessage
    lcd.setCursor(0, 2);
    lcd.print(BEACON); // Beacon text

    sendmsg(BEACON); 
    
    lcd.setCursor(0, 2);
    lcd.print("BEACON DELAY        "); // Delay  text
...
void splash_RX_freq()
{
    ...
    String frequency_str = "Freq: ";
    frequency_str += RX_frequency;
    char frequency_chr[frequency_str.length()+1];                         //create char buffer
    frequency_str.toCharArray(frequency_chr, frequency_str.length()+1);   //convert to char
    IOShieldOled.setCursor(0, 2);
    IOShieldOled.putString(frequency_chr);
    ...
    lcd.setCursor(11, 3);
    lcd.print(MEMORY);
    stringSPEED = String(CW_SPEED, DEC);
    lcd.setCursor(18, 3);
    lcd.print(stringSPEED);
    
    String speed_str = "";
    speed_str += CW_SPEED;
    char speed_chr[speed_str.length()+1];                     //create char buffer
    speed_str.toCharArray(speed_chr, speed_str.length()+1);   //convert to char
    IOShieldOled.setCursor(9, 0);
    IOShieldOled.putString(speed_chr);
...
void dash() {
  audioState = digitalRead(audioSwitch);
  if (audioState == HIGH) { 
  tone(tonePin,CW_PITCH); 
  }
